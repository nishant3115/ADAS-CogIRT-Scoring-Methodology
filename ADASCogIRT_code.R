...
# load example cross-sectional data to build IRT model
load(file = 'Cross_section_ADAS_Data.RData')

# training the IRT model using mirt package
# define factor loadings (F1-memory, F2-language, F3-praxis)
# All 3 traits allowed to be correlated with each other (defined by COV = F1*F2, F2*F3, F1*F3)
...
# define the interaction terms between patient covariates and ADAS-Cog items
# Defined based on our measurement invariance analysis (refer to published paper)
...
# ADAS_Cross is cross-sectional ADAS-Cog data, Cross_Covariates are patient covariate terms
Cross_Model_F3 <- 
...
dat_Covariates$RID <- as.character(dat_Covariates$RID) # unique patient roster ID
...
Patient_RID <- c(); Patient_Visits <- c(); Patient_Arm <- c(); Patient_Age <- c();
Item_Model <- c(1:35)
for(it in 1:length(Item_Model)){
  ...
  for(unq in 2:(length(uniq_item))){
    ...
    # slopes associated with memory (theta1), language (theta2), and praxis (theta3) latent traits
    Theta1_Slopes <- c(Theta1_Slopes, (F3_Slopes_Not_CM[item, 1]*(1-dat_Covariates$CM_Med) +
                                         F3_Slopes_CM[item, 1]*dat_Covariates$CM_Med))
    Theta2_Slopes <- c(Theta2_Slopes, (F3_Slopes_Not_CM[item, 2]*(1-dat_Covariates$CM_Med) +
                                         F3_Slopes_CM[item, 2]*dat_Covariates$CM_Med))
    Theta3_Slopes <- c(Theta3_Slopes, (F3_Slopes_Not_CM[item, 3]*(1-dat_Covariates$CM_Med) +
    ...
    M_e3 <- which(dat_Covariates$Gender==0) # Check the pattern of encoding of gender (COMMON MISTAKE)
...
GLMER_F3_Data <- data.frame(Theta1_Slopes, Theta2_Slopes, Theta3_Slopes,
                            ...
                            Patient_Arm, Patient_RID, Patient_VISCODE, Patient_EDU, Patient_Age, Patient_Gender,
                            Item_Responses, Theta_Intercept, Patient_Visits, 
                            HU200 = (Patient_Arm=='HU200ug')*1, 
                            HU400 = (Patient_Arm=='HU400ug')*1)

GLMER_F3_Data$Patient_EDU <- (GLMER_F3_Data$Patient_EDU-mean(GLMER_F3_Data$Patient_EDU))/
  sd(GLMER_F3_Data$Patient_EDU)
GLMER_F3_Data$Patient_Age <- (GLMER_F3_Data$Patient_Age-mean(GLMER_F3_Data$Patient_Age))/
...
HU_GLMER_F3 <- glmer(Item_Responses ~ 0 + 
                       ...
                       Theta1_Rate + Theta2_Rate + Theta3_Rate + 
                       I(Theta1_Slopes*HU200) + I(Theta2_Slopes*HU200) + I(Theta3_Slopes*HU200) + 
                       I(Theta1_Rate*HU200) + I(Theta2_Rate*HU200) + I(Theta3_Rate*HU200) + 
                       I(Theta1_Slopes*HU400) + I(Theta2_Slopes*HU400) + I(Theta3_Slopes*HU400) + 
                       I(Theta1_Rate*HU400) + I(Theta2_Rate*HU400) + I(Theta3_Rate*HU400) + 
                       I(Theta1_Rate*Patient_Age) + I(Theta2_Rate*Patient_Age) + I(Theta3_Rate*Patient_Age) +                              
                       I(Theta1_Slopes*Patient_Age) + I(Theta2_Slopes*Patient_Age) + I(Theta3_Slopes*Patient_Age) +
